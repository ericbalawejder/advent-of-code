...
public class AdapterArray {
  ...
  public static void main(String[] args) throws IOException {
    final SortedSet<Integer> outputJoltage = readFileToTreeSet("src/main/java/aoc/year2020/day10/output-joltage.txt");
  ...
  static long countAllPaths(Graph<Integer, DefaultEdge> directedGraph, SortedSet<Integer> sortedAdapters) {
    ...
    final List<GraphPath<Integer, DefaultEdge>> longestPath = paths
        .getAllPaths(adapters.first(), adapters.last(), true, null);
  ...
  private static SortedSet<Integer> readFileToTreeSet(String path) {
    try (Stream<String> stream = Files.lines(Paths.get(path))) {
      final SortedSet<Integer> adapters = stream.map(Integer::parseInt)
          .collect(Collectors.toCollection(TreeSet::new));
